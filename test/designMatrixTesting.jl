...
mat = MatrixLM.design_matrix(f=@mlmFormula( catvar1 + catvar2 + catvar3 + x1 + x2 + x3 + x4 ),df=X_df,
               cntrst=[(:catvar1, DummyCoding()) (:catvar2, :catvar3, EffectsCoding()) ]  )
mat2 = MatrixLM.design_matrix(f=@mlmFormula(catvar1 + catvar2 + catvar3 + x1 + x2 + x3 + x4),df = X_df, cntrst=methods)
